...
class _GaussianPermutationCMICache:
    """Safe cached Gaussian CMI evaluator for permutation tests."""
    def __init__(self, X, Y, Z=None):
        self.X = np.atleast_2d(X)
        if self.X.shape[0] == 1 and self.X.shape[1] > 1:
            self.X = self.X.T
        self.Y = np.atleast_2d(Y) if Y is not None else None
        if self.Y is not None and self.Y.shape[0] == 1 and self.Y.shape[1] > 1:
            self.Y = self.Y.T
        self.Z = np.atleast_2d(Z) if Z is not None else None
        if self.Z is not None and self.Z.shape[0] == 1 and self.Z.shape[1] > 1:
            self.Z = self.Z.T

        self.n = self.X.shape[0]
        self.kx = self.X.shape[1]
        self.ky = self.Y.shape[1] if self.Y is not None else 0
        self.kz = self.Z.shape[1] if self.Z is not None else 0
        ...
        XYZ = np.hstack(tuple(parts))
        self.corr_xyz = np.corrcoef(XYZ.T)

        idx_x = np.arange(0, self.kx)
        idx_y = np.arange(self.kx, self.kx + self.ky) if self.Y is not None else np.array([], dtype=int)
        idx_z = np.arange(self.kx + self.ky, self.kx + self.ky + self.kz) if self.Z is not None else np.array([], dtype=int)

        self.idx_z = idx_z
        self.idx_xz = np.concatenate((idx_x, idx_z)) if self.Z is not None else idx_x
        self.idx_yz = np.concatenate((idx_y, idx_z)) if self.Z is not None and self.Y is not None else idx_y
        self.idx_xyz = np.arange(0, self.kx + self.ky + self.kz)
    ...
    def evaluate(self, idx):
        ...
        if (
            ...
            or not np.array_equal(np.sort(idx), np.arange(self.n))
        ):
            return gaussian_conditional_mutual_information(self.X[idx], self.Y, self.Z)

        # Re-index X using idx while keeping Y and Z fixed
        X_perm = self.X[idx]
        return gaussian_conditional_mutual_information(X_perm, self.Y, self.Z)
